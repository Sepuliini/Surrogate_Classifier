...
parser = argparse.ArgumentParser(description='Run DTLZ surrogates')
...
def save_optimization_results(solutions, problem_name, algorithm_name, ea_name, num_vars, num_obj, num_samples, output_folder):
    # Create the directory to save optimization results
    ...
    # Construct the filename to include number of objectives, variables, and samples
    results_filename = path.join(
        optimization_results_dir,
        f"{algorithm_name}_{ea_name}_{num_obj}obj_{num_vars}vars_{num_samples}samples_optimization_results.csv"
    )

    # Convert the solutions to a DataFrame
